...
**목표 상태(to-be)인가 현재 상태(as-is)인가.** 목표 상태로 두면 갱신 의무가 없고, 코드가
앞서 나가면 그림이 틀린 것이 아니라 코드가 도달한 것이다. 현재 상태로 두면 갱신 의무가
생기고, 지킬 수 없으면 그리지 않는 편이 낫다. `references/diagrams-readme.md` 를 본다.
...
- 목표 구조에서 코드에 없는 것은 `미착륙`(빨강 점선)으로 표시하고, **기준 시점을 제목에
  적는다**. 시점 없는 미착륙 표시는 아무도 걷어내지 못한다.
